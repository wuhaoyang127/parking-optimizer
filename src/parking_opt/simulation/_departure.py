...
class DepartureMixin:
    ...
    def _vehicle_departure(self, vehicle: Vehicle, dep_time: float):
        ...
        if not blockers:
            ...
            self.parking_lot.free(spot, self.env.now)
        ...
        for blk_spot, blk_vid in blockers:
            ...
            # 阻挡车就近前移归位（同组最内侧空闲位；没有再回原位）。
            # 回位前/后各检查一次：行驶期间若已被其它进程移走（并发让行竞态），
            # 则跳过回位，防止 move_vehicle 断言崩溃。
            if buffer.occupied_by != blk_vid:
                self.parking_lot.release_buffer(buffer.spot_id)
                self._clear_shift_level(blk_vid)
                continue
            innermost = self.parking_lot.find_innermost_available(blk_spot)
    ...
    def _find_innermost_available(self, spot: Spot) -> Spot | None:
        """找纵深组中最内侧的空闲车位（用于前移归位；统一走 ParkingLot 实现）。"""
        return self.parking_lot.find_innermost_available(spot)
