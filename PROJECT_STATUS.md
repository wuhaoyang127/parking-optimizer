...
- 2026-08-21：打备份 tag `backup-before-feedback-round2-20260821`；完成反馈优化第二批 `stage-d-feedback-round2`：需求时序条形图/车辆明细表/需求序列 JSON 导出导入 + 指标权重百分条/归一化/加权排名（与字典序并存切换），pytest 49 passed，自检与 AppTest 通过；后续补充需求序列「另存为…（原生保存对话框，自选位置）」+「手动输入路径保存」+「从项目文件夹直接选择导入」+「一键把浏览器下载的 parking_demand*.json 移动到项目文件夹」（打 tag `backup-before-demand-save-20260821`），待用户验收；
