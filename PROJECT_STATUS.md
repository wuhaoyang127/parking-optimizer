...
- **当前阶段阻断项**：无；
- **当前唯一下一步**：本地计算后端全链路已本地验证通过（worker 已带网络自愈重试）；公网 app 本地计算模式已提供「📥 下载 worker 一键启动脚本 / 开机自启安装脚本」。剩余：用户在公网 app 下载脚本并双击启动 worker 后，走一遍「💻 本地计算」→ 下发任务 → 结果自动载入。
