...
- **当前唯一下一步**：在 Supabase 执行 `migrations/15_algo_push.sql`（algorithm_releases 发布快照表 + 实验表幂等 + 自定义权限新功能位 can_push_algo）→ ☁️ 云端重新部署 → 🚀 验收「新算法接入 → 🚀 推送到车主端」区块（手动选定推送 / 自动推送当前测试最优 / 发布与实验记录；自定义角色功能开关出现「推送到车主端」）；随后企业交付前人工待办：删旧 publishable key、正式交付前改管理员强密码、数据授权协议、试点基线、`docs/企业可用性检查清单.md`。
