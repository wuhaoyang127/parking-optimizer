...
- **当前阶段阻断项**：无；
- **当前唯一下一步**：修复导入布局「不持久」问题——诊断确认布局已存在 Supabase，但恢复路径把网络异常静默吞掉，抖动时布局不加载；已加恢复重试（3 次）+ 页面渲染自愈（30 秒节流）+ 失败可见与「重试加载云端布局」按钮，121 项测试与启动包自检通过，待用户在公网 app 验收。
