...
9. 🎛️ 验收策略选择两段式（经典/规则 vs 机器学习）：仿真设置页先选「是否加入机器学习」再选策略；「否，仅经典/规则算法」下拉含 9 个经典算法 + 全部对比，「是，加入机器学习算法」下拉当前含 RHO + 全部对比；「全部对比」只跑当前分类内算法（云端与本地计算一致）；本地 `py -m streamlit run app.py` 可先自测。
