...
current_milestone: stage-d-compare-checkboxes
git_initialized: true
current_branch: stage-d-deliver
last_verified_commit: 0a471e0
...
- **当前里程碑**：`stage-d-dynamic-path-feedback`（动态路径页反馈修复：入库虚线按实际入口、离场/移位动画、移位车辆表；多入口多出口已验收；已于 2026-08-28 用户验收通过）；布局图「内置/真实布局」回显 + 指标分析需求时序按策略切换，已于 2026-08-28 用户验收通过；反馈按修改后显示时间自动排序（含正序/倒序切换），已于 2026-08-28 用户验收通过；**全库 Python 文件已按功能拆分至单文件 ≤200 行**（AGENTS.md 3.2.1 已写入强制规则：源码/测试/脚本全部适用，超 200 行必须按功能拆，一个功能一个文件；`scripts/check_file_lines.py` 自检通过，143 测试通过），已于 2026-09-02 完成；**权限体系全面重做**（9 板块可见性 + 14 项功能权限逐按钮开关：新增本地计算/删除本地任务/导入需求/导出需求/导出结果/删除历史/提交反馈/反馈管理；内置三角色按推荐矩阵：管理员全开、操作员含本地计算与历史删除、访客仅运行+反馈；「自定义」角色可同时勾选板块和功能权限，permissions 存 {sections, features}；迁移 13 最终版 + 迁移 14 旧数据升级），已于 2026-09-02 完成并**用户验收通过**；**动态路径移位修复**（离场段起点改为按事件流重放车辆实际车位：移位后未回位从缓冲位出发、已回位从回位目标出发；移位阶段新增「回位段」可选动画；引擎 `move_vehicle` 同步 `assigned_spot`、`free` 释放缓冲位占用），已于 2026-09-02 完成；**回放路径假直线与内置布局单向边修复**（回放路径找不到真实路线时按引擎口径回退入口、绝不画穿越空地的假直线；内置 linear/rectangle/circle 三布局补齐入口反向边——此前车能进不能出，离场动画无路可画；新增回归测试，全部内置布局每个车位必须能返回入口），已于 2026-09-02 完成，待用户验收；**策略选择两段式改造**（先选「是否加入机器学习」再选策略；`BaseStrategy.category` 分类：按严格口径全部现有算法含 RHO 统计预测均归经典、ml 分类暂空；「全部对比」只对比当前分类内算法，云端/本地口径一致），已于 2026-09-14 完成，待用户网页验收；**🎯 自动调参**（单策略一键试 10 组参数选最优并回填控件；全部对比可选「默认参数 + 最优参数」两组对比；云端/本地计算口径一致），已于 2026-09-14 完成，待用户网页验收；**💾 本地任务结果回传压缩**（worker 回传前把 events_by_strategy/vehicles_by_strategy/main_events 三个大字段 gzip+base64 压缩、网页载入本地结果时自动解压，旧格式无 `_packed` 标记原样返回；修复 1000 车 compare_all 任务回传 complete_compute_task 时触发 Supabase 语句超时 57014 的 bug；失败任务 `46ae1108` 按用户要求保持 failed 不重跑），已于 2026-09-16 完成，待下次本地计算时验证；**🧮 计算内容三动作拆分 + K 可输入 + >20 分批调参**（把「自动调参选最优」「用最优参数跑仿真/排序」「按当前参数跑仿真/排序」拆成三个可独立勾选的复选框，可只调参、可只跑当前参数、也可都选；调参组数 K 改为数字输入默认 10、最少 5、无上限，K>20 时每 20 个一批批内选优再比各批最优；本地 worker 与云端口径一致，旧任务 auto_tune/tune_compare 字段向后兼容），已于 2026-09-16 完成，待用户网页验收；**🚗 缓冲位评分选位 + 二次移位惩罚 + 就近回位**（移位选缓冲位不再按字典序取第一个，改为综合代价 `C = w_d·距离D̂ + w_t·空闲时间T̂ + w_q·二次移位Q̂` 取最小：D̂=移位车原车位到候选缓冲位的路网最短距离候选集内 min–max 归一化；T̂=1/(1+已空闲时长/τ)，τ 默认 300s 半衰，等待队列非空时 +0.3 压力，车位新增 `last_freed_at` 由 free/move_vehicle 携带时刻更新；Q̂=候选为纵深外层位且同组内层有车记 1（会挡内层车）、独立位/内层全空记 0；默认权重 w_d=1.0/w_t=1.0/w_q=2.0，允许二次移位但惩罚更重——引擎 `_shift_level` 跟踪车辆移位层级，SHIFT_START 事件带 `shift_level`/`buffer_score`，回位统一「同组最内侧空闲位」就近前移（场景 A/B 一致）；指标新增 `secondary_shift_count`/`secondary_shift_distance_m` 单列，对比表加「二次移位」列，单策略降级提示含二次移位；权重与 τ 做成环境参数控件贯通云端/本地/调参/worker，旧任务缺省值向后兼容），已于 2026-09-16 完成，待用户网页验收；
- **当前阶段阻断项**：无（迁移 13/14 已直连执行并只读核验：`users.permissions` 列、`app_settings.custom_sections` 模板（当前 9 板块 + 13 功能开）与 login_user / validate_session / list_users / update_user_role / get_custom_sections / save_custom_sections 六个 RPC 均已就位；迁移 15/16 已于 2026-09-25 直连只读核验在库中生效：`algorithm_releases`/`experiments`/`experiment_assignments`/`experiment_events` 四表与 publish_algorithm / list_algorithm_releases / list_experiments 三 RPC 均存在；`users.username` 四处外键（user_preferences/feedback/sim_runs/compute_tasks）均为 ON UPDATE CASCADE，change_username / admin_rename_user 两 RPC 均存在且含 `wuhaoyang127` 保护）；
- **当前唯一下一步**：本地/云端网页验收「策略对比自由勾选」（仿真设置页策略下拉选「全部对比」→ 多选框勾选任意算法，经典与机器学习可混合，如 贪心 + ML 时长预测树 同场对比；单算法下拉不再分两段式，ML 算法带「（机器学习）」标记）；☁️ 云端重新部署后公网抽验；随后企业交付前人工待办：删旧 publishable key、正式交付前改管理员强密码、数据授权协议、试点基线、`docs/企业可用性检查清单.md`。
...
- [x] 阶段 D：🚀 推送到车主端（`migrations/15_algo_push.sql`：`algorithm_releases` 发布快照表 + experiments/experiment_assignments/experiment_events 三表幂等补建 + RPC `publish_algorithm`/`list_algorithm_releases`/`list_experiments`（SQL 层仅管理员）+ 自定义角色权限新增 `can_push_algo` 并同步旧模板；`src/auth/releases.py` RPC 封装；「新算法接入」页新增「🚀 推送到车主端」区块（🎯 选定算法推送 / 🧠 自动推送当前测试最优——优先最近一次「全部对比」否则单策略平均加权排名 / 📜 发布与实验记录；`src/ui/pages/algo_push.py` 纯函数 `_best_algo_from_runs` 可测）；权限矩阵：管理员默认开、操作员/访客默认关、自定义角色可勾选；`pyproject.toml` 依赖拆分 core（networkx/simpy）/ui/baseline extras 供车主端轻量安装；`docs/车主端接入契约.md` 约定车主端 FastAPI 安装包、读发布快照、在线 assign 契约、实验分流、指标上报与保留/回滚），pytest 183 passed、自检通过（≤200 行/文件）、启动包校验通过，迁移 15 已于 2026-09-25 直连只读核验在库中生效（4 表 + 3 RPC 均存在），并于 2026-09-25 用户网页验收通过；**车主端接入开关**：车主端 FastAPI 未接入前，推送按钮仅备用（点击提示「车主端未接入」不写库），接入后设环境变量 `OWNER_APP_CONNECTED=1` 重新部署即启用真实推送）；
- [x] 阶段 D：✏️ 用户名修改（`migrations/16_change_username.sql`：DO 块给 user_preferences/feedback/sim_runs/compute_tasks 引用 users(username) 的外键补 `ON UPDATE CASCADE`；RPC `change_username`（本人自改）/`admin_rename_user`（管理员改任意用户），长度 2~32、唯一冲突捕获、审计日志，`wuhaoyang127` 管理员账号不可改名；`src/auth/users.py` 新增 `_clean_username`/`change_username`/`admin_rename_user`；侧边栏新增「✏️ 修改用户名」折叠面板，系统设置用户管理行新增 ✏️ 改名按钮），pytest 188 passed、自检通过，迁移 16 已于 2026-09-25 直连只读核验在库中生效（四处外键 ON UPDATE CASCADE + change_username/admin_rename_user 两 RPC 均存在且含 wuhaoyang127 保护），并于 2026-09-25 用户网页验收通过；
- [x] 阶段 D：算法四 ml_duration_tree 接入（首个机器学习算法，category=ml，ml 分类下拉自动出现）：①新增 `src/parking_opt/strategies/_cart.py` 纯 numpy CART 回归树（叶均值/方差减少分裂/最大深度与叶最小样本限制，不新增依赖）；②新增 `src/parking_opt/strategies/ml_duration.py`——prepare 离线用全量需求训练「到达时刻/入口 → 停车时长」回归树（标签为真实时长、特征只用在线可观测，与 MOSA 同为离线训练+在线执行口径），assign 在线用模型预测时长替代 estimated_duration 做内外层分流、里层选位也用预测时长（信息源一致），预测无区分度（ptp≤均值 5%）自动退化为外层优先贪心；参数 max_depth=4/min_samples_leaf=5/use_entry=true；③登记注册表后「是否加入机器学习=是」策略下拉自动出现「ML 时长预测树」及 3 个参数控件；④实验 `scripts/exp_ml_duration.py`（100 车位/300 车/5 seeds 均值，评分与网页一致的加权口径）：场景 A（时长与到达无关，默认需求模型）ML 无信息可学、排名第 4（诚实展示）；场景 B（时长与到达时段相关，真实感）ML 加权分 0.3420 超过主方法 duration_greedy 0.3175 与 greedy 0.3219、排名第 2（nearest 第 1）；计算耗时 ML 比主方法多约 1.2~1.4s（含训练，可接受）；⑤新增 `tests/test_ml_duration.py`（10 项：树常数/分段拟合/叶样本限制 + 注册/ml 分类/无参构造/参数声明 + 退化回退/时段分流/预测非负/入口 one-hot）并更新 `tests/test_strategy_categories.py`（ml 分类不再为空），pytest 243 passed、行数检查与启动包自检全绿（289 关键文件），待云端部署后网页验收；
...
| `src/local_compute/_tuning.py` | 自动调参纯函数（PARAMS 随机采样/修复/按排名口径选最优） | 有效，本轮新增 |
| `src/local_compute/_groups.py` | 多种子分组运行纯函数（云端/本机共用口径） | 有效，本轮新增 |
| `src/local_compute/_compress.py` | 本地结果大字段 gzip+base64 压缩/解压（回传防语句超时） | 有效，本轮新增 |
| `tests/test_result_compress.py` | 结果压缩/解压回归测试（8 项） | 有效，本轮新增 |
| `tests/test_worker_compute.py` | worker 三动作组合（只调参/只跑/调参+最优跑）回归测试（5 项） | 有效，本轮新增 |
| `tests/test_buffer_scoring.py` | 缓冲位评分选位 + 二次移位层级 + 就近回位回归测试（9 项） | 有效，本轮新增 |
| `src/ui/pages/auto_tune.py` | 单策略自动调参（云端运行/结果回填/摘要表） | 有效，本轮新增 |
| `src/ui/pages/compare_all_run.py` | 云端全部对比：默认参数组 + 最优参数组 | 有效，本轮新增 |
| `tests/test_auto_tune.py` | 自动调参纯函数测试（10 项） | 有效，本轮新增 |
...
| `src/parking_opt/strategies/_cart.py` | CART 回归树（纯 numpy，供 ML 策略使用） | 有效，本轮新增 |
| `src/parking_opt/strategies/ml_duration.py` | 算法四 ML 时长预测树（离线训练/在线预测，ml 分类） | 有效，本轮新增 |
| `tests/test_ml_duration.py` | 算法四 10 项单元/回归测试 | 有效，本轮新增 |
| `scripts/exp_ml_duration.py` | 算法四第一版实验脚本（产出 `outputs/exp_ml_duration_summary.md`） | 有效，本轮新增 |
...
> 💾 2026-09-16：本地任务结果回传压缩已修复（worker 回传前把大字段 gzip+base64 压缩，网页载入本地结果时自动解压，旧格式兼容）。失败任务 `46ae1108`（1000 车 compare_all+自动调参）按用户要求**保持 failed 不重跑**；想继续算时：先用新代码重启本地 worker，再把该任务改回 pending（或网页重新下发同参数任务）即可，回传不会再因 result 过大触发 57014 语句超时。
>
> 🧮 2026-09-16：计算内容已拆为三动作复选框（按当前参数跑 / 自动调参选最优 / 用最优参数跑），K 可输入（默认 10、最少 5、无上限，>20 每 20 个一批）；本地 `py -m streamlit run app.py` 可先自测，云端需重新部署后网页验收。

> 🚗 2026-09-16：缓冲位评分选位已实现（`C = w_d·距离 + w_t·空闲时间 + w_q·二次移位` 取最小，默认 1.0/1.0/2.0、τ=300s；SHIFT_START 带 `shift_level`/`buffer_score`；回位就近前移；指标新增二次移位单列）。本地 `py -m streamlit run app.py` 可先自测：仿真设置页环境参数出现 4 个新滑杆（缓冲位-距离权重/空闲时间权重/二次移位惩罚/空闲半衰），指标页对比表出现「二次移位」列；云端需重新部署后网页验收。
...
9. 🎛️ 验收策略选择两段式（经典/规则 vs 机器学习）：仿真设置页先选「是否加入机器学习」再选策略；「否，仅经典/规则算法」下拉含全部 10 个现有算法 + 全部对比；「是，加入机器学习算法」当前显示「暂无机器学习算法」占位（ml 分类为空，运行按钮置灰）；「全部对比」只跑当前分类内算法（云端与本地计算一致）；本地 `py -m streamlit run app.py` 可先自测。
10. 🎯 验收自动调参：①单策略模式（如时长感知贪心）出现「🎯 自动调参（试 10 组）」按钮，点击后跑 10 组单种子仿真、按当前排名设置选最优并回填控件，摘要表列出各组表现；②全部对比勾选「全部对比时自动调参」后，指标页出现「默认参数对比」+「🎯 最优参数对比」两组，附「查看各算法最优参数」折叠表；③「💻 本地计算」模式下调参任务由本机 worker 执行（worker 窗口可见 `[🎯] 调参 xxx` 进度）。
...
- 2026-09-25：**策略对比改为自由勾选（跨经典/ML 分类混合对比）**（用户需求：想选哪几个算法就对比哪几个，ML 也可与贪心等同场对比）：①`StrategyRegistry` 新增 `items_filtered(names, category)`——勾选列表优先、分类回退（旧任务兼容）、都给 None 返回全部，云端/本机 worker 共用口径；②仿真设置页策略选择重构：去掉两段式「是否加入机器学习」radio，改为**单算法下拉（跨分类，ML 算法带「（机器学习）」标记）**；下拉选「全部对比」时出现「☑️ 勾选参与对比的算法」多选框（默认全选、可任意勾选、可混合经典与 ML），勾空则运行按钮置灰；③云端 `compare_all_run` / 本地任务 payload（新增 `strategy.names`）/ `worker_compute` 全部改为按勾选列表取策略，旧任务无 names 仍按 category 回退（向后兼容）；④新增/更新测试：`items_filtered` 跨分类混合与优先级回退（2 项）、worker 按 names 过滤（1 项），pytest 246 passed、行数检查与启动包自检全绿；打备份 tag `backup-before-compare-checkboxes-20260925`；状态文档 status_version 89 → 90、`current_milestone` → `stage-d-compare-checkboxes`；待用户网页验收；
- 2026-09-25：**算法四 ml_duration_tree 接入（首个机器学习算法）**（用户需求：先加一个 ML 算法看计算速度与评分，评分标准与无 ML 一致，看 ML 能否优化）：①`src/parking_opt/strategies/_cart.py` 纯 numpy CART 回归树（无新依赖）；②`src/parking_opt/strategies/ml_duration.py`——prepare 离线训练「到达时刻/入口→停车时长」回归树（标签真实时长、特征在线可观测），assign 在线预测时长做内外层分流、里层选位同用预测（信息源一致），预测无区分度（ptp≤均值 5%）退化外层优先贪心；参数 max_depth/min_samples_leaf/use_entry；③登记注册表后 ml 分类自动出现（「是否加入机器学习=是」下拉）；④实验 `scripts/exp_ml_duration.py`（100 车位/300 车/5 seeds，网页同口径加权评分）：场景 A 时长与到达无关→ML 无信息可学排名第 4（诚实）；场景 B 时长与时段相关→ML 加权分 0.3420 超主方法 duration_greedy 0.3175 与 greedy 0.3219、排名第 2；ML 耗时比主方法多约 1.2~1.4s（含训练）；⑤新增 `tests/test_ml_duration.py`（10 项）+ 更新分类测试，pytest 243 passed、行数与启动包自检全绿（289 关键文件）；状态文档 status_version 88 → 89、`current_milestone` → `stage-d-ml-duration-tree`；待云端部署后网页验收；
- 2026-09-25：**用户网页验收通过（验收债清零）**：迁移 15/16（算法推送区块 + 用户名修改：侧边栏自改、管理员改名、`wuhaoyang127` 保护）+ 9-16 三批（🚗 缓冲位评分选位 4 滑杆与「二次移位」列 / 🧮 计算内容三动作拆分 + K 可输入 / 💾 本地结果回传压缩）全部验收通过；状态文档 status_version 87 → 88、唯一下一步更新为「接入第一个机器学习算法」；打备份 tag `backup-before-ml-algo-20260925`；
- 2026-09-25：**验收债清理：迁移 15/16 只读核验生效 + 代码推送**（用户选择「先清验收债」）：①直连 Supabase 只读核验——迁移 15 对象已在库（`algorithm_releases`/`experiments`/`experiment_assignments`/`experiment_events` 四表 + `publish_algorithm`/`list_algorithm_releases`/`list_experiments` 三 RPC）；迁移 16 对象已在库（`user_preferences`/`feedback`/`sim_runs`/`compute_tasks` 四处 `username` 外键均为 `ON UPDATE CASCADE`，`change_username`/`admin_rename_user` 两 RPC 存在且含 `wuhaoyang127` 保护，`audit_log` 表在位）；②本地 HEAD `0a471e0`（缓冲位评分选位）已推送到 origin/stage-d-deliver（此前领先 1 个 commit）；③本地三项自检全绿：pytest 233 passed、`scripts/check_file_lines.py` 全部 ≤200 行、启动包自检 285 关键文件；④打备份 tag `backup-before-status-migration16-verified-20260925`；状态文档 status_version 86 → 87、`current_milestone` 更新为 `stage-d-username-change`、`current_exec_plan` 置 null；下一步为云端重新部署 + 网页验收；
- 2026-09-16：**🚗 缓冲位评分选位 + 二次移位惩罚 + 就近回位**（用户反馈：移位时为何不优先选最近车位，且要考虑车位空闲时间、二次移位要加惩罚、回位可就近）：①`Spot` 新增 `last_freed_at`（最近释放时刻），`ParkingLot.free/move_vehicle` 带 `now` 时更新（默认 None 向后兼容）；②`ParkingLot.select_buffer_scored` 新增综合代价选位 `C = w_d·D̂ + w_t·T̂ + w_q·Q̂`——D̂ 原车位到候选缓冲位的路网最短距离候选集内 min–max 归一化；T̂=1/(1+已空闲时长/τ)、τ 默认 300s、等待队列非空时 +0.3 压力、从未占用视作 0；Q̂ 候选为纵深外层且同组内层有车记 1（会挡内层车）、独立位/内层全空记 0；默认权重 1.0/1.0/2.0，`select_buffer()` 旧字典序逻辑保留向后兼容；③`SimulationEngine` 新增 `buffer_w_distance/buffer_w_idle/buffer_w_secondary/buffer_idle_half_life` 参数与 `_select_buffer` 封装、`_shift_level` 车辆移位层级跟踪（next/commit/rollback/clear），场景 A/B 移位全部改走评分选位，SHIFT_START 事件带 `shift_level`/`buffer_score`，失败/竞态分支回退层级；④回位统一就近前移：场景 B 从「回原位」改为 `find_innermost_available` 同组最内侧空闲位（与场景 A 一致，方法上收到 ParkingLot，DepartureMixin 委托保留兼容）；⑤指标 `compute_metrics` 新增 `secondary_shift_count`/`secondary_shift_distance_m`（level≥2 单列），`COUNT_FIELDS` 同步取整，UI 对比表（加权/字典序）加「二次移位」列、单策略降级提示含二次移位；⑥环境参数 `ENV_PARAM_SPECS` 新增 4 个滑杆（距离权重/空闲权重/二次移位惩罚/空闲半衰 τ），`cloud_run/compare_all_run/auto_tune/task_context/local_task_ops/worker_compute` 全链路贯通，旧任务缺省值向后兼容；⑦新增 `tests/test_buffer_scoring.py`（9 项：距离优先/空闲时间/队列压力/二次移位惩罚/最后释放时刻/就近回位/场景 B 回位/二次移位 level+metrics/旧接口兼容）；pytest 233 passed、行数检查与启动包自检全绿（285 关键文件）；打 tag `backup-before-buffer-scoring-20260916`；状态文档 status_version 85 → 86；待用户网页验收；
- 2026-09-16：**计算内容三动作拆分 + K 可输入 + >20 分批调参**（用户需求：把「自动调参选最优」和「调整参数求最优」分开，可只调参、可只跑当前参数、也可都选；K 默认 10、最少 5、无上限，>20 时每 20 个一批批内选优再比各批最优）：①`src/local_compute/_tuning.py` 新增 `TUNE_TRIALS_MIN=5`/`TUNE_BATCH_SIZE=20` 与 `resolve_strategy_flags`（新任务 run_default/auto_tune/tune_run/tune_trials 四字段，旧任务 tune_compare/auto_tune 向后兼容）；`run_tuning` 改为分批选优（>20 时每 20 个一批，批内 best_trial 后批间再 best_trial，同 seed 指标直接比较不重跑）；②`src/local_compute/_groups.py` 新增 `tune_all_strategies`（对一组策略各自调参，worker/云端共用，支持进度回调）；③`worker_compute.py` 重写三动作调度——单策略可只调参（回传最优参数不跑仿真）、调参+最优跑、按当前参数跑、或两者都跑（先当前后最优，`default_metrics` 留存当前参数结果）；compare_all 可只调参/只默认组/默认+最优组，events 优先默认组否则最优组；④UI：`layout_strategy.py` 去掉「全部对比时自动调参」复选框，改为「🎛️ 自动调参组数 K」数字输入（min 5 无上限）；`settings_page.py` 新增「🧮 计算内容」三复选框（按当前参数跑 / 🎯 自动调参选最优 / 🚀 用最优参数跑，tune_run 依赖 auto_tune）；`cloud_run.py` 支持 rerun_after=False 连续跑多组；`compare_all_run.py` 三动作调度 + 只调参时显示摘要不跳页；`auto_tune.py` 调参只回填不自动跑，摘要支持全部对比各算法最优参数表；⑤本地任务 payload 改用四字段，`local_task_actions.py` 载入只调参结果时仅回填参数+摘要不跳指标页；⑥新增 `tests/test_worker_compute.py`（5 项）+ `test_auto_tune.py` 扩充分批/兼容/常量测试（+7 项）；pytest 224 passed、行数检查与启动包自检全绿（284 关键文件）；打 tag `backup-before-tune-split-20260916`；待用户网页验收；
- 2026-09-16：**本地任务结果回传压缩（修复 57014 语句超时）**（用户反馈：本地任务失败、算完回传时丢结果）：诊断 Supabase 最近失败任务 `46ae1108`（wuhaoyang127，1000 车 compare_all+自动调参，真实布局 250 车位），错误 `canceling statement due to statement timeout`（57014，数据库 statement_timeout=2min）——worker 已算完但 result 过大（events_by_strategy/vehicles_by_strategy/main_events 合计约 8.4M 字符）回传 UPDATE 超时，result 未落库；修复：①新增 `src/local_compute/_compress.py` 纯函数——worker 回传前把三个大字段 gzip+base64 打包并加 `_packed` 标记，网页载入本地结果时 `decompress_result` 自动解压，旧格式（无标记）原样返回；②`local_worker.py` 回传前调 `compress_result`；③`src/ui/pages/local_task_actions.py` `_apply_sim_state` 入口解压（载入最近结果/轮询完成两条路径共用）；④`local_compute/__init__.py` 与 `ui/common/_imports.py` 导出；⑤新增 `tests/test_result_compress.py`（8 项：JSONB 往返一致/None 字段/旧格式兼容/体积减小/无副作用/幂等/非 dict 安全/worker 可导入）；pytest 213 passed、行数检查与启动包自检全绿；打 tag `backup-before-result-compress-20260916`；按用户要求失败任务保持 failed 不重跑，待下次本地计算时验证；
- 2026-09-14：**🎯 自动调参（Auto Tuning）**（用户需求：不再手动试参数，点按钮自动试 K 组选最优；K=10 已确认；全部对比增加「默认参数 + 最优参数」两组对比；支持本地计算）：①新增 `src/local_compute/_tuning.py` 纯函数——按 PARAMS 声明随机采样（int 按 step 粒度、float/choice/strategy/bool 均匀随机、locked 不采、RHO mild≤severe 修复），单种子仿真后按当前排名口径（加权 `weighted_rank` 或字典序）选最优；②新增 `src/local_compute/_groups.py` 纯函数 `run_group`——多种子分组运行统一口径，云端与本地 worker 共用（消除两处 compare_all 逻辑重复）；③`worker_compute.py` 重写：单策略 `auto_tune` 任务只调参回传、compare_all 的 `tune_compare` 先逐算法调参再跑默认+最优两组；④云端拆分 `src/ui/pages/compare_all_run.py`（全部对比含调参进度条）与 `auto_tune.py`（单策略调参/结果回填/摘要表），`cloud_run.py` 瘦身为单策略入口；⑤仿真设置页：单策略显示「🎯 自动调参（试 10 组）」按钮，compare_all 显示「全部对比时自动调参」复选框（默认勾选，可取消）；调参完成后最优参数自动回填控件并在摘要表列出各组表现；⑥指标分析页：最优参数组以「🎯 最优参数对比」单独成表，附「查看各算法最优参数」折叠表；⑦新增 `tests/test_auto_tune.py`（10 项）；pytest 205 passed、行数检查与启动包自检全绿；待用户网页验收；
- 2026-09-14：**RHO 归类修正 + 策略选择 UI 优化**（用户质疑 RHO 是否真属机器学习）：按严格口径，RHO 的 SARIMA-lite（Holt + AR(1)）属统计时序预测而非机器学习，从 ml 归回 classic；ml 分类暂空（框架保留，接入真正的树模型/神经网络/强化学习后自动出现）；网页在 ml 分类为空时显示「🧠 暂无机器学习算法」占位并置灰运行按钮；`strategy_options_for` 空分类返回空列表；同步精简 radio 帮助文案与 caption、删除明显冗余注释；更新 `docs/新算法接入说明.md` 分类口径与 `tests/test_strategy_categories.py`（ml 为空、RHO 归 classic），pytest 195 passed、行数自检通过；待用户网页验收；
- 2026-09-14：**策略选择两段式改造（经典/规则 vs 机器学习）**（用户要求后续接入 ML 算法，先选是否加入机器学习、再选策略）：①策略基类 `BaseStrategy` 新增 `category` 分类（`CATEGORY_CLASSIC`=经典/规则默认、`CATEGORY_ML`=机器学习），常量与分类查询 `names_in_category`/`items_in_category` 落在 `strategies/registry.py`；②现有算法归类：RHO（SARIMA-lite 时序预测）归 `ml`，FCFS/最近/随机/贪心/离场贪心/时长感知贪心/融合示例/MOSA（NSGA-II 进化搜索）/risk_scoring（多准则评分）归 `classic`——判定口径：从数据中学习规律才算 ML，进化算法/多准则评分不算；③仿真设置页策略选择改两段式：先 radio「是否加入机器学习」再按分类出策略下拉，「全部对比」在每个分类下都出现且只对比当前分类内算法（`strategy_options_for` 纯函数）；④云端 `_run_cloud_simulation` 与本地 `worker_compute.run_local_task` 的 compare_all 都按 payload/选择里的 category 过滤（旧任务无 category 保持全部对比，向后兼容）；⑤新增 `tests/test_strategy_categories.py`（7 项）、更新 `docs/新算法接入说明.md`（category 约定与归类口径）；pytest 195 passed、行数检查与启动包自检全绿；打 tag `backup-before-strategy-ml-category-20260914`；待用户网页验收；
